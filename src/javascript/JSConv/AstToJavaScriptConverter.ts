import { Visitor } from "esprima-next";

export class AstToJavaScriptConverter extends Visitor {
    ...
    static readonly s_lastSwitchCaseFlag = {};
    static readonly s_forLoopInitDeclarationFlag = {};
    static readonly s_bindingPatternAllowsExpressionsFlag = {}; // automatically propagated to sub-patterns

    private readonly _ignoreExtensions: boolean;

    private _writeContext: WriteContext;
    private _currentStatementFlags: StatementFlags;
    private _currentExpressionFlags: ExpressionFlags;
    private _currentAuxiliaryNodeContext: object;

    constructor(writer: JavaScriptTextWriter, options: AstToJavaScriptOptions) {
        this.Writer = writer;

        _ignoreExtensions = options.IgnoreExtensions;
    }

    public Writer: JavaScriptTextWriter

    protected WriteContext: WriteContext

    //protected  ParentNode :Node

    public Convert(node: Node) {
